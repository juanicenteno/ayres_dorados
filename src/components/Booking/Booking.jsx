...
const GuestsSelector = ({ label, count, setCount, min = 0 }) => (
  <div className={styles.AdultsMenu}>
    ...
    <div className={styles.AddDelete}>
      ...
      >
        <path d="M6 12L18 12" stroke="#b08d57" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
      <small>{count}</small>
      <svg
        role="button"
...
function Booking({ hotelId = "3176", roomId = "12899" }) {
  const hoy = new Date();
  ...
  // Animación drawer mobile: esperar a que se abra antes de mostrar contenido
  useEffect(() => {
    let timer;
    if (searchOpen) {
  ...
  const generarEnlaceReserva = () => {
    if (!date1 || !date2) return notifyError("Completá la fecha de entrada y salida.");
    if (adultsCount === 0) return notifyError("Seleccioná al menos 1 adulto.");
    if (date2 <= date1) return notifyError("La salida debe ser posterior al ingreso.");
  ...
  // Cerrar menú de huéspedes al hacer click fuera
  ...
  // Texto de huéspedes para el trigger desktop
  const guestLabel = `${adultsCount} adult${adultsCount !== 1 ? "os" : "o"}${childrenCount > 0 ? `, ${childrenCount} niño${childrenCount !== 1 ? "s" : ""}` : ""}`;
    ...
    <>
          ...
          <>
            <section className={styles.calendars}>
              <DatePicker
                ...
                onChange={(val) => {
                  setDate1(val);
                  // Si checkout queda antes que check-in, lo corregimos automáticamente
                  if (date2 && val >= date2) {
              ...
              <DatePicker
                ...
                value={date2}
                onChange={setDate2}
                min={date1 ? (() => { const d = new Date(date1); d.setDate(d.getDate() + 1); return formatDate(d); })() : fechaManana}
                max={fechaMax}
              />
            </section>

            <section className={styles.guests}>
              <GuestsSelector label="Adultos" count={adultsCount} setCount={setAdultsCount} min={1} />
              <GuestsSelector label="Niños" count={childrenCount} setCount={setChildrenCount} min={0} />
            </section>
      ...
      <section className={styles.bookingDesktop}>
        ...
        <section className={styles.guestsDesktop}>
          ...
          >
            <span className={styles.guestText}>{guestLabel}</span>
            <svg
